...
// Database connection
mongoose.connect("mongodb://3.109.184.255:27017/exam_center", {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (error.code === 11000) {
        res.status(400).json({
          message:
            "A franchise with the same name and location already exists.",
        })
      } else {
        // Handle other errors
        res.status(500).json({ message: error.message })
...
app.get(
  "/franchises/associated",
  authenticate(["user", "admin", "superadmin"]),
  async (req, res) => {
    try {
      if (req.user.role === "user") {
        ...
        if (!user.franchise) {
          return res
            .status(404)
            .json({ message: "No franchise associated with this user" })
        }
...
app.post(
  "/api/logout",
  authenticate(["superadmin", "admin", "user"]),
  (req, res) => {
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "Franchise deleted successfully", franchise })
    } catch (error) {
      res.status(500).json({ message: error.message })
    }
  }
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      const franchise = await Franchise.findByIdAndUpdate(id, updatedData, {
        new: true,
      })
      if (!franchise) {
        return res.status(404).json({ message: "Franchise not found" })
...
app.post(
  "/api/franchises/:franchiseId/exam-data",
  authenticate(["user", "admin", "superadmin"]),
  async (req, res) => {
    ...
    try {
      ...
      if (!examData || !Array.isArray(examData) || examData.length === 0) {
        return res
          .status(400)
          .json({ message: "Exam data must be a non-empty array." })
      }
      ...
      if (req.user.role !== "superadmin") {
        ...
        const isFranchiseAssociated = user.franchise.some(
          (franchise) => franchise._id.toString() === franchiseId
        )
        if (!isFranchiseAssociated) {
          return res
            ...
            .json({
              message:
                "You are not authorized to add exam data to this franchise.",
            })
        }
      }
      ...
      const newExamData = examData.map((data) => {
        ...
        if (!name || !examName || !date || !durationInMinutes || !status) {
          throw new Error(
            "All fields (name, examName, date, durationInMinutes, and status) are required."
          )
        }
        ...
        if (!validExamNames.includes(examName)) {
          throw new Error(
            `Invalid exam name. Allowed values are: ${validExamNames.join(
              ", "
            )}`
          )
        }
        ...
        if (!validDurations.includes(duration)) {
          throw new Error(
            `Invalid duration. Allowed values are: ${validDurations.join(", ")}`
          )
        }
...
app.put(
  "/franchises/:franchiseId/exam-data/:examDataId",
  authenticate(["user", "admin", "superadmin"]),
  async (req, res) => {
    ...
    try {
      ...
      if (user.role === "superadmin") {
        ...
        if (!isFranchiseAssociated) {
          return res
            ...
            .json({
              message:
                "You are not authorized to update exam data for this franchise.",
            })
        }
      }
      ...
      if (!validExamNames.includes(examName)) {
        return res
          .status(400)
          .json({
            message: `Invalid exam name. Allowed values are: ${validExamNames.join(
              ", "
            )}`,
          })
      }
      ...
      if (!validDurations.includes(duration)) {
        return res
          .status(400)
          .json({
            message: `Invalid duration. Allowed values are: ${validDurations.join(
              ", "
            )}`,
          })
      }
      ...
      res
        ...
        .json({
          ...
          examData: franchise.examData.id(examDataId),
        })
    } catch (error) {
      console.error(
        "Error updating exam data at franchiseId:",
        franchiseId,
        "ExamDataId:",
        examDataId,
        "Error:",
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (user.role === "superadmin") {
        ...
        if (!isFranchiseAssociated) {
          return res
            ...
            .json({
              message:
                "You are not authorized to delete exam data for this franchise.",
            })
        }
      }
      ...
      const examDataIndex = franchise.examData.findIndex(
        (data) => data._id.toString() === examDataId
      )
      if (examDataIndex === -1) {
        return res.status(404).json({ message: "Exam data not found." })
      }
  ...
  async (req, res) => {
    try {
      ...
      if (userRole === "superadmin") {
        const allFranchiseExamData = await Franchise.find().select(
          "name location examData"
        )
        if (!allFranchiseExamData || allFranchiseExamData.length === 0) {
          return res
            .status(404)
            .json({ message: "No exam data found for any franchises" })
        }
      ...
      if (!user.franchise || user.franchise.length === 0) {
        return res
          .status(404)
          .json({ message: "No associated franchises found for this user" })
      }
      ...
      if (!franchiseExamData || franchiseExamData.length === 0) {
        return res
          .status(404)
          .json({ message: "No exam data found for associated franchises" })
      }
            ...
            examsByDay: {
              $sum: {
                $cond: [
                  {
                    $and: [
                      { $gte: ["$examData.date", startDate || startOfToday] },
                      { $lt: ["$examData.date", endDate || endOfToday] },
                    ],
                  },
                  1,
                  0,
            ...
            examsByWeek: {
              $sum: {
                $cond: [
                  {
                    $and: [
                      { $gte: ["$examData.date", startOfWeek] },
                      { $lt: ["$examData.date", endOfWeek] },
                    ],
                  },
                  1,
                  0,
            ...
            examsByMonth: {
              $sum: {
                $cond: [
                  {
                    $and: [
                      { $gte: ["$examData.date", startOfMonth] },
                      { $lt: ["$examData.date", endOfMonth] },
                    ],
                  },
                  1,
                  0,
