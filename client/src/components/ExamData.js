    ...
    const fetchDurationOptionsList = async () => {
      try {
        ...
        if (response && Array.isArray(response)) {
          setDurationList(
            response.filter((duration) => typeof duration === "number")
          )
        } else {
  ...
  const handleDelete = async (examId, franchiseId) => {
    try {
      ...
      setExamDataList((prevState) =>
        prevState.filter((exam) => exam.examId !== examId)
      )
    ...
    try {
      ...
      setExamDataList((prevState) =>
        prevState.map((examItem) =>
          examItem.examId === editingExamId
            ? { ...examItem, ...updatedExam }
            : examItem
        )
      )
      setFilteredExamData((prevState) =>
                      ...
                      <td style={styles.td}>
                        {editingExamId === exam.examId ? (
                          ...
                          <>
                            ...
                            <button
                              onClick={() =>
                                handleDelete(exam.examId, exam.franchiseId)
                              }
